...
export default function Home() {
  ...
  return (
    <div className="h-screen w-screen flex flex-col justify-between items-center relative">
      <Head>
        <title>Better Pokemon</title>
      </Head>
      <div className="text-2xl text-center pt-8">Which Pokémon is Better?</div>
      {pokemonPair && (
        <div className="p-8 flex justify-between items-center max-w-2xl flex-col md:flex-row animate-fade-in">
          <PokemonListing
            pokemon={pokemonPair.firstPokemon}
            vote={() => voteForBetter(pokemonPair.firstPokemon.id)}
            disabled={fetchingNext}
          />
          <div className="p-8 italic text-xl">{"or"}</div>
          <PokemonListing
            pokemon={pokemonPair.secondPokemon}
            vote={() => voteForBetter(pokemonPair.secondPokemon.id)}
            disabled={fetchingNext}
          />
          <div className="p-2" />
        </div>
